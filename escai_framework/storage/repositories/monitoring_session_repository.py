    ...
    async def get_session_duration_stats(
        ...
        result = await session.execute(
            select(
                ...
                func.avg(
                    func.extract('epoch', MonitoringSession.ended_at - MonitoringSession.started_at)
        ...
        return {
            ...
            'min_duration_seconds': float(row.min_duration_seconds) if row.min_duration_seconds else 0.0,
            'max_duration_seconds': float(row.max_duration_seconds) if row.max_duration_seconds else 0.0
        }
